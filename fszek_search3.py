...
driver.get("http://saman.fszek.hu/WebPac/CorvinaWeb?action=advancedsearchpage")
time.sleep(3)
...
for i in range(len(writerList)): 
    ...
    driver.find_element_by_id("text1").clear() # clear szerzo
    driver.find_element_by_id("text2").clear() # clear cim
    driver.find_element_by_id("text1").click() # click szerzo
    driver.find_element_by_id("text1").send_keys(writer) # inputs the writer
    ...
    try:
        ...
        print(talalat)
        talalatok.append(writer + " : " + talalat)
        #if there is talaltok search for the novels
        driver.find_element_by_id("text2").click()
        driver.find_element_by_id("text2").clear()
        #start loop in listOfNovelsList
        time.sleep(3)
        for j in range(len(listOfNovelsList)): 
            novel = listOfNovelsList[i][j]
            print ("actual novel:" + novel) 
            driver.find_element_by_id("text2").clear()
            driver.find_element_by_id("text2").send_keys(novel)
            driver.find_element_by_id("searchform").submit()
            try:
                talalatNovel = driver.find_element_by_xpath("//td[contains(@class, 'thead_2of5')]").text #talált dokumentum
                talalatok.append(novel + " : " + talalatNovel)
            except:
                talalatok.append(novel + " nincs találat ")
    except:
        print("nincs találat: " + writer)
        talalatok.append(writer + " : nincs találat ")

time.sleep(3)   # 3 sec delay
print("************ WEB RUNNING ENDS *****************")
